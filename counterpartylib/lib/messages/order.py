...
def cancel_order_match (db, order_match, status, block_index):
    ...
    if tx0_order['status'] in ('expired', 'cancelled'):
        ...
        if tx0_order['get_asset'] == config.BTC and (block_index >= 0 or config.TESTNET or config.REGTEST):    # Protocol change.
    ...
    if tx1_order['status'] in ('expired', 'cancelled'):
        ...
        if tx1_order['get_asset'] == config.BTC and (block_index >= 0 or config.TESTNET or config.REGTEST):    # Protocol change.
    ...
    # Penalize tardiness.
    if block_index >= 0 or config.TESTNET or config.REGTEST:  # Protocol change.
...
def validate (db, source, give_asset, give_quantity, get_asset, get_quantity, expiration, fee_required, block_index):
    ...
    if expiration == 0 and not (block_index >= 0 or config.TESTNET or config.REGTEST):   # Protocol change.
...
def match (db, tx, block_index=None):
    ...
    if tx['block_index'] > 0 or config.TESTNET or config.REGTEST:  # Protocol change.
    ...
    for tx0 in order_matches:
        ...
        if tx0_price > tx1_inverse_price:
            ...
            fee = 0
            if block_index >= 0 or config.TESTNET or config.REGTEST: # Protocol change. Deduct fee_required from provided_remaining, etc., if possible (else don’t match).
            ...
            # tx0
            tx0_status = 'open'
            if tx0_give_remaining <= 0 or (tx0_get_remaining <= 0 and (block_index >= 0 or config.TESTNET or config.REGTEST)):    # Protocol change
            ...
            log.message(db, block_index, 'update', 'orders', bindings)
            # tx1
            if tx1_give_remaining <= 0 or (tx1_get_remaining <= 0 and (block_index >= 0 or config.TESTNET or config.REGTEST)):    # Protocol change
