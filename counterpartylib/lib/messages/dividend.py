...
def validate (db, source, quantity_per_unit, asset, dividend_asset, block_index):
    ...
    if asset == config.XCP:
        if (not block_index >= 0) or block_index >= 0 or config.TESTNET or config.REGTEST:   # Protocol change.
...
def parse (db, tx, message):
    ...
    # Unpack message.
    try:
        if (tx['block_index'] > 0 or config.TESTNET or config.REGTEST) and len(message) == LENGTH_2:
